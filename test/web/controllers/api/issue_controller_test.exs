defmodule PitchIn.Web.IssueControllerTest do
  ...
  test "lists all entries on index", %{conn: conn} do
    conn = get conn, api_issue_path(conn, :index)
    assert json_response(conn, 200)["data"] == []
  end

  test "creates issue and renders issue when data is valid", %{conn: conn} do
    conn = post conn, api_issue_path(conn, :create), issue: @create_attrs
    assert %{"id" => id} = json_response(conn, 201)["data"]

    conn = get conn, api_issue_path(conn, :show, id)
    assert json_response(conn, 200)["data"] == %{
      "id" => id,
      "value" => "some value"}
  end
  ...
  test "does not create issue and renders errors when data is invalid", %{conn: conn} do
    conn = post conn, api_issue_path(conn, :create), issue: @invalid_attrs
    assert json_response(conn, 422)["errors"] != %{}
  end

  test "updates chosen issue and renders issue when data is valid", %{conn: conn} do
    %Issue{id: id} = issue = fixture(:issue)
    conn = put conn, api_issue_path(conn, :update, issue), issue: @update_attrs
  ...
  test "does not update chosen issue and renders errors when data is invalid", %{conn: conn} do
    issue = fixture(:issue)
    conn = put conn, api_issue_path(conn, :update, issue), issue: @invalid_attrs
    assert json_response(conn, 422)["errors"] != %{}
  end

  test "deletes chosen issue", %{conn: conn} do
    issue = fixture(:issue)
    conn = delete conn, api_issue_path(conn, :delete, issue)
    assert response(conn, 204)
    assert_error_sent 404, fn ->
      get conn, api_issue_path(conn, :show, issue)
    end
  end
end
